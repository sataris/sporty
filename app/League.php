...
class League extends Model
{
    ...
    {
        $client = new Guzzle();
        $response = $client->get('http://api.stats.foxsports.com.au/3.0/api/sports/afl/series/1/seasons/118/fixturesandresultswithbyes.json?userkey=A00239D3-45F6-4A0A-810C-54A347F144C2');
        ...
        try {
            ...
            foreach ($result as $match) {
                ...
                $gameMatch->teams()->sync([$teamA->id => ['is_winner' => $teamAWin , 'super_goals' => $match->team_A->superGoals, 'goals' => $match->team_A->goals, 'behinds' => $match->team_A->behinds, 'points' => $match->team_A->score]], false);
                $gameMatch->teams()->sync([$teamB->id => ['is_winner' => $teamBWin , 'super_goals' => $match->team_B->superGoals, 'goals' => $match->team_B->goals, 'behinds' => $match->team_B->behinds, 'points' => $match->team_B->score]], false);
            }
        } catch (Exception $e) {
            dd($e->getMessag());
        }
